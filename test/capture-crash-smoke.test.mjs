// test/capture-crash-smoke.test.mjs — CAP-05: 캡처 직후 강제 종료해도 유실이 없음을
...
test('캡처 직후 강제 종료해도 재기동 시 그 캡처가 메인 창 목록에 있다', async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'whennote-crash-smoke-'));
  ...
  try {
    ...
    assert.ok(
      Array.isArray(renderer.titles) && renderer.titles.includes(marker),
      `재기동 후 메인 창 목록에서 표식을 찾지 못했다: ${JSON.stringify(renderer.titles)}\n` +
        `--- p2 stdout ---\n${done.stdout}`
    );
  } finally {
