...
- **D-06 기본 단축키 `Ctrl+Alt+N`**: WHENWORK의 `Ctrl+Alt+` 체계와 같은 손 모양이고 충돌하지 않는다. macOS 조합은 platform 모듈이 WHENWORK와 같은 규칙으로 정한다. 폐기안 `Ctrl+Shift+Space`(목업 표기)·`Ctrl+Alt+M`.
  - **개정(2026-09-16)**: 단축키를 둘로 나눈다 — 퀵 메모 `Ctrl+N`, 메모 창 `Ctrl+Alt+N`(사용자 결정). `Ctrl+N`은 다른 앱의 "새 파일·새 창"과 겹치는 전역 조합이라 그 앱들에서 `Ctrl+N`이 막힌다는 점을 사용자에게 알렸고, 설정에서 각각 바꿀 수 있다. 둘에 같은 조합을 주면 뒤에 등록하는 쪽이 실패해 화면에 드러난다(`lifecycle.applyHotkeys`).
