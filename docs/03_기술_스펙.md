...
| # | 내용 | 선택지 | 추천 |
|---|---|---|---|
| 5 | 클립보드 API 세대 교체 | Phase 3 구현 시점에 (a) 현재 동기 API 사용 후 Electron 업그레이드 때 대응 / (b) 처음부터 `await`로 감싼 어댑터 한 곳에 격리 | (b). `main/clipboard.mjs` 한 파일에 `readText`·`readImage`를 감싸 두면 어느 세대든 호출부가 바뀌지 않는다. 구현 직전 context7로 그 시점 버전의 문서를 다시 확인 |
| 6 | 임시 앱 아이콘 | `assets/icon/whennote.png`는 스크립트로 만든 임시 이미지(앰버 배경 + 흰 메모지). 사용자가 준비한 실제 아이콘(파란 배경 + 흰 메모지·시계)으로 교체 필요 | 파일을 받으면 교체 후 `npm run icons`, `test/icon.test.mjs`로 트레이 글리프 확인 |
